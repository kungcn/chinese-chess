...
<script>
    export default {
        ...
        mounted() {
           ...
           // 初始化游戏
           let ChessGame = window.ChessGame = (id) => {
            ...
            ChessGame.prototype = {
                ...
                drawChessboard: () => {
                    ...
                    let r = 9;
                    let c = 8;
                    let cellSize = that.chessboard.cellSize;
                    let w = c * cellSize;
                    let h = r * cellSize;
                    ...
                    function drawX(r, c, pos) {
                        ...
                        ctx.strokeStyle = "#000";
                        if (pos[0]) {
                            drawTopLeft(x, y, that.chessboard.markLen, that.chessboard.markPadding);
                        }
                        if (pos[1]) {
                            drawTopRight(x, y, that.chessboard.markLen, that.chessboard.markPadding);
                        }
                        if (pos[2]) {
                            drawBottomLeft(x, y, that.chessboard.markLen, that.chessboard.markPadding);
                        }
                        if (pos[3]) {
                            drawBottomRight(x, y, that.chessboard.markLen, that.chessboard.markPadding);
                        }
                        ctx.stroke();
                    }

                    ctx.beginPath();
                    ctx.lineWidth = that.chessboard.lineWidth;
                    ctx.strokeStyle = that.chessboard.strokeStyle;
            ...
            let cg = new ChessGame("chessCanvas");
            cg.renderChessBoard(true);
