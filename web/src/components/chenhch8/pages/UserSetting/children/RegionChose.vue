...
<mu-popup position="bottom"
    popupClass="region-chose"
    :open="status"
    @close="close()" >
  <div id="region-chose-popup"
...
export default {
  ...
  created: function() {
    this.address = [ this.region.province, this.region.city ];
    this.province = this.address[0];
    this.city = this.address[1];
    this.addressSlots[1].values = address[this.province]
  },
  computed: {
    ...mapGetters({
       status: 'getRegionPopupStatus',
